...
namespace FSSystem.ContentAdapter.HPEAndHPInc.Parser {
    public static class DetailValueParser {
        private static readonly Lazy<Regex> dimensionRegex = new Lazy<Regex>(new Regex(
            @"([(]*(?<dim1>l|h|d|w)\s*x\s*(?<dim2>l|h|d|w)\s*x\s*(?<dim3>l|h|d|w)[)]*)*\s*
...
\s*(?<uom>cm|mm|m|in|inch|inches|"")*",
        ...
        private static readonly Lazy<Regex> weightRegex = new Lazy<Regex>(new Regex(
            @"(?<weight>\d*\.?\d+)\s*(?<unitOfMeasure>lbs?|oz?|kg?|g?)",
        ...
        public static (decimal? weight, WeightUnitOfMeasure uom) TryParseWeight(string input) {
            try {
                ...
                var actualWeight = decimal.Parse(weight?.Value ?? "",
        ...
        private static decimal? GetHeight(string[] dims, string[] measures) {
            ...
            if (decimal.TryParse(measures[pos], NumberStyles.AllowDecimalPoint | NumberStyles.AllowThousands,
                CultureInfo.InvariantCulture, out var retVal)) {
                return retVal;
            }
